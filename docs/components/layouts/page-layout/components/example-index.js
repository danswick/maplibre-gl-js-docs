...
import { AsideHeading } from '@mapbox/dr-ui/on-this-page/helpers';
import Card from '@mapbox/dr-ui/card';
import { levels } from '@mapbox/dr-ui/level-indicator';
    ...
    filterPages = () => {
        ...
        if (search) {
            ...
            filteredPages = filteredPages.filter(
                (f) =>
                    (f.title &&
                        f.title.toLowerCase().indexOf(trimmedSearch) > -1) ||
                    (f.description &&
